// Language translations for Image Cube
const translations = {
    'en': {
        // Navigation
        'nav.home': 'Home',
        'nav.compress': 'Compress',
        'nav.convert': 'Convert',
        'nav.crop': 'Crop',

        // Hero Section
        'hero.badge': '✨ 100% Free · No Registration · Privacy Safe',
        'hero.title': 'Smart Image Processing',
        'hero.subtitle': 'All-in-One Solution',
        'hero.tagline': 'Lossless Compression · Format Conversion · Precise Cropping',
        'hero.description': 'All processing is done locally in your browser. Images never leave your device.',
        'hero.description2': 'Your privacy, we take it seriously.',
        'hero.cta': 'Get Started',

        // Feature Cards
        'feature.compress.title': 'Smart Compression',
        'feature.compress.desc': 'Advanced compression algorithms maintain visual quality while significantly reducing file size. Batch processing supported.',
        'feature.compress.btn': 'Start Compressing →',
        'feature.convert.title': 'Format Conversion',
        'feature.convert.desc': 'Convert between JPG, PNG, WebP, GIF and more. One-click conversion to optimal format.',
        'feature.convert.btn': 'Start Converting →',
        'feature.crop.title': 'Precise Cropping',
        'feature.crop.desc': 'Free crop or choose preset ratios. Supports passport photos and common sizes.',
        'feature.crop.btn': 'Start Cropping →',

        // Stats
        'stats.speed': 'Fast',
        'stats.speed.label': 'Local Processing',
        'stats.privacy': '100%',
        'stats.privacy.label': 'Privacy Protected',
        'stats.free': 'Forever',
        'stats.free.label': 'Free to Use',

        // Compress Tool
        'compress.title': '🗜️ Image Compression',
        'compress.desc': 'Smart compression algorithm maintains visual quality while significantly reducing file size',
        'compress.quality': 'Compression Quality',
        'compress.quality.high': 'High Compression',
        'compress.quality.recommended': 'Recommended',
        'compress.quality.best': 'Best Quality',
        'compress.maxSize': 'Max Size Limit',
        'compress.noLimit': 'No Limit',

        // Convert Tool
        'convert.title': '🔄 Format Conversion',
        'convert.desc': 'Convert between multiple image formats for different use cases',
        'convert.format': 'Target Format',
        'convert.quality': 'Output Quality',
        'convert.webp.desc': 'Smallest Size',
        'convert.jpeg.desc': 'Best Compatibility',
        'convert.png.desc': 'Supports Transparency',

        // Crop Tool
        'crop.title': '✂️ Image Cropping',
        'crop.desc': 'Precise cropping with multiple preset ratios and free cropping',
        'crop.ratio': 'Aspect Ratio',
        'crop.ratio.free': 'Free',
        'crop.presets': 'Common Sizes',
        'crop.idPhotos': 'ID Photo Sizes',
        'crop.presets.1inch': '1-inch (25×35mm)',
        'crop.presets.2inch': '2-inch (35×49mm)',
        'crop.presets.passport': 'Passport (35×45mm)',
        'crop.presets.visa': 'Visa (33×48mm)',
        'crop.presets.usVisa': 'US Visa (51×51mm)',
        'crop.format': 'Output Format',
        'crop.format.original': 'Keep Original',
        'crop.selectOther': 'Select Other Image',
        'crop.reset': 'Reset',
        'crop.apply': 'Crop & Download',

        // Upload Area
        'upload.text': 'Drag images here, or click to select files',
        'upload.hint.compress': 'Supports JPG, PNG, WebP, GIF formats',
        'upload.hint.convert': 'Supports JPG, PNG, WebP, GIF, BMP formats',
        'upload.hint.crop': 'Supports JPG, PNG, WebP formats',
        'upload.btn': 'Select Images',

        // Results
        'result.title': 'Processing Results',
        'result.reprocess': '🔄 Reprocess',
        'result.downloadAll': 'Download All',
        'result.original': 'Original',
        'result.new': 'New',
        'result.download': 'Download',

        // Processing
        'processing.text': 'Processing...',

        // Footer
        'footer.copyright': '© 2025 Image Cube - Professional Online Image Processing Tool',
        'footer.note': 'All processing is done locally in your browser. Your images are never uploaded.'
    },

    'zh-CN': {
        // Navigation
        'nav.home': '首页',
        'nav.compress': '压缩',
        'nav.convert': '转换',
        'nav.crop': '裁切',

        // Hero Section
        'hero.badge': '✨ 100% 免费 · 无需注册 · 隐私安全',
        'hero.title': '智能图片处理',
        'hero.subtitle': '一站式解决方案',
        'hero.tagline': '无损压缩 · 格式转换 · 精准裁切',
        'hero.description': '所有处理均在浏览器本地完成，图片不会上传到任何服务器',
        'hero.description2': '您的隐私，我们认真对待',
        'hero.cta': '立即开始',

        // Feature Cards
        'feature.compress.title': '智能压缩',
        'feature.compress.desc': '先进的压缩算法，在保持视觉质量的同时大幅减小文件大小，支持批量处理',
        'feature.compress.btn': '开始压缩 →',
        'feature.convert.title': '格式转换',
        'feature.convert.desc': '支持 JPG、PNG、WebP、GIF 等多种格式互转，一键转换为最优格式',
        'feature.convert.btn': '开始转换 →',
        'feature.crop.title': '精准裁切',
        'feature.crop.desc': '自由裁切或选择预设比例，支持1寸、2寸证件照等常用尺寸',
        'feature.crop.btn': '开始裁切 →',

        // Stats
        'stats.speed': '极速',
        'stats.speed.label': '本地处理',
        'stats.privacy': '100%',
        'stats.privacy.label': '隐私保护',
        'stats.free': '永久',
        'stats.free.label': '免费使用',

        // Compress Tool
        'compress.title': '🗜️ 图片压缩',
        'compress.desc': '智能压缩算法，在保持视觉质量的同时大幅减小文件大小',
        'compress.quality': '压缩质量',
        'compress.quality.high': '高压缩',
        'compress.quality.recommended': '推荐',
        'compress.quality.best': '高质量',
        'compress.maxSize': '最大尺寸限制',
        'compress.noLimit': '无限制',

        // Convert Tool
        'convert.title': '🔄 格式转换',
        'convert.desc': '支持多种图片格式互转，满足不同场景需求',
        'convert.format': '目标格式',
        'convert.quality': '输出质量',
        'convert.webp.desc': '体积最小',
        'convert.jpeg.desc': '兼容性好',
        'convert.png.desc': '支持透明',

        // Crop Tool
        'crop.title': '✂️ 图片裁切',
        'crop.desc': '精准裁切，支持多种预设比例和自由裁切',
        'crop.ratio': '裁切比例',
        'crop.ratio.free': '自由',
        'crop.presets': '常用尺寸',
        'crop.idPhotos': '证件照尺寸',
        'crop.presets.1inch': '1寸 (25×35mm)',
        'crop.presets.2inch': '2寸 (35×49mm)',
        'crop.presets.passport': '护照 (35×45mm)',
        'crop.presets.visa': '签证 (33×48mm)',
        'crop.presets.usVisa': '美国签证 (51×51mm)',
        'crop.format': '输出格式',
        'crop.format.original': '保持原格式',
        'crop.selectOther': '选择其他图片',
        'crop.reset': '重置',
        'crop.apply': '裁切并下载',

        // Upload Area
        'upload.text': '拖拽图片到这里，或点击选择文件',
        'upload.hint.compress': '支持 JPG、PNG、WebP、GIF 格式',
        'upload.hint.convert': '支持 JPG、PNG、WebP、GIF、BMP 格式',
        'upload.hint.crop': '支持 JPG、PNG、WebP 格式',
        'upload.btn': '选择图片',

        // Results
        'result.title': '处理结果',
        'result.reprocess': '🔄 重新处理',
        'result.downloadAll': '下载全部',
        'result.original': '原始',
        'result.new': '新',
        'result.download': '下载',

        // Processing
        'processing.text': '正在处理...',

        // Footer
        'footer.copyright': '© 2025 图片魔方 - 专业的在线图片处理工具',
        'footer.note': '所有处理均在浏览器本地完成，不会上传您的图片'
    },

    'zh-TW': {
        // Navigation
        'nav.home': '首頁',
        'nav.compress': '壓縮',
        'nav.convert': '轉換',
        'nav.crop': '裁切',

        // Hero Section
        'hero.badge': '✨ 100% 免費 · 無需註冊 · 隱私安全',
        'hero.title': '智能圖片處理',
        'hero.subtitle': '一站式解決方案',
        'hero.tagline': '無損壓縮 · 格式轉換 · 精準裁切',
        'hero.description': '所有處理均在瀏覽器本地完成，圖片不會上傳到任何伺服器',
        'hero.description2': '您的隱私，我們認真對待',
        'hero.cta': '立即開始',

        // Feature Cards
        'feature.compress.title': '智能壓縮',
        'feature.compress.desc': '先進的壓縮演算法，在保持視覺品質的同時大幅減小檔案大小，支援批量處理',
        'feature.compress.btn': '開始壓縮 →',
        'feature.convert.title': '格式轉換',
        'feature.convert.desc': '支援 JPG、PNG、WebP、GIF 等多種格式互轉，一鍵轉換為最優格式',
        'feature.convert.btn': '開始轉換 →',
        'feature.crop.title': '精準裁切',
        'feature.crop.desc': '自由裁切或選擇預設比例，支援1吋、2吋證件照等常用尺寸',
        'feature.crop.btn': '開始裁切 →',

        // Stats
        'stats.speed': '極速',
        'stats.speed.label': '本地處理',
        'stats.privacy': '100%',
        'stats.privacy.label': '隱私保護',
        'stats.free': '永久',
        'stats.free.label': '免費使用',

        // Compress Tool
        'compress.title': '🗜️ 圖片壓縮',
        'compress.desc': '智能壓縮演算法，在保持視覺品質的同時大幅減小檔案大小',
        'compress.quality': '壓縮品質',
        'compress.quality.high': '高壓縮',
        'compress.quality.recommended': '推薦',
        'compress.quality.best': '高品質',
        'compress.maxSize': '最大尺寸限制',
        'compress.noLimit': '無限制',

        // Convert Tool
        'convert.title': '🔄 格式轉換',
        'convert.desc': '支援多種圖片格式互轉，滿足不同場景需求',
        'convert.format': '目標格式',
        'convert.quality': '輸出品質',
        'convert.webp.desc': '體積最小',
        'convert.jpeg.desc': '相容性好',
        'convert.png.desc': '支援透明',

        // Crop Tool
        'crop.title': '✂️ 圖片裁切',
        'crop.desc': '精準裁切，支援多種預設比例和自由裁切',
        'crop.ratio': '裁切比例',
        'crop.ratio.free': '自由',
        'crop.presets': '常用尺寸',
        'crop.idPhotos': '證件照尺寸',
        'crop.presets.1inch': '1吋 (25×35mm)',
        'crop.presets.2inch': '2吋 (35×49mm)',
        'crop.presets.passport': '護照 (35×45mm)',
        'crop.presets.visa': '簽證 (33×48mm)',
        'crop.presets.usVisa': '美國簽證 (51×51mm)',
        'crop.format': '輸出格式',
        'crop.format.original': '保持原格式',
        'crop.selectOther': '選擇其他圖片',
        'crop.reset': '重置',
        'crop.apply': '裁切並下載',

        // Upload Area
        'upload.text': '拖曳圖片到這裡，或點擊選擇檔案',
        'upload.hint.compress': '支援 JPG、PNG、WebP、GIF 格式',
        'upload.hint.convert': '支援 JPG、PNG、WebP、GIF、BMP 格式',
        'upload.hint.crop': '支援 JPG、PNG、WebP 格式',
        'upload.btn': '選擇圖片',

        // Results
        'result.title': '處理結果',
        'result.reprocess': '🔄 重新處理',
        'result.downloadAll': '下載全部',
        'result.original': '原始',
        'result.new': '新',
        'result.download': '下載',

        // Processing
        'processing.text': '正在處理...',

        // Footer
        'footer.copyright': '© 2025 圖片魔方 - 專業的線上圖片處理工具',
        'footer.note': '所有處理均在瀏覽器本地完成，不會上傳您的圖片'
    }
};

// Language detection and management
class I18n {
    constructor() {
        this.currentLang = this.detectLanguage();
        this.listeners = [];
    }

    detectLanguage() {
        // Check localStorage first
        const saved = localStorage.getItem('imagecube-lang');
        if (saved && translations[saved]) {
            return saved;
        }

        // Detect from browser
        const browserLang = navigator.language || navigator.userLanguage;

        if (browserLang.startsWith('zh')) {
            // Distinguish between Simplified and Traditional Chinese
            if (browserLang === 'zh-TW' || browserLang === 'zh-HK' || browserLang === 'zh-Hant') {
                return 'zh-TW';
            }
            return 'zh-CN';
        }

        // Default to English
        return 'en';
    }

    setLanguage(lang) {
        if (translations[lang]) {
            this.currentLang = lang;
            localStorage.setItem('imagecube-lang', lang);
            document.documentElement.lang = lang;
            this.updatePage();
            this.notifyListeners();
        }
    }

    t(key) {
        return translations[this.currentLang][key] || translations['en'][key] || key;
    }

    updatePage() {
        // Update all elements with data-i18n attribute
        document.querySelectorAll('[data-i18n]').forEach(el => {
            const key = el.getAttribute('data-i18n');
            el.textContent = this.t(key);
        });

        // Update all elements with data-i18n-placeholder
        document.querySelectorAll('[data-i18n-placeholder]').forEach(el => {
            const key = el.getAttribute('data-i18n-placeholder');
            el.placeholder = this.t(key);
        });

        // Update document title based on language
        const titles = {
            'en': 'Image Cube - Free Online Image Processing Tool',
            'zh-CN': '图片魔方 - 免费在线图片处理工具',
            'zh-TW': '圖片魔方 - 免費線上圖片處理工具'
        };
        document.title = titles[this.currentLang];

        // Update language switcher display
        this.updateLanguageSwitcher();
    }

    updateLanguageSwitcher() {
        const currentDisplay = document.getElementById('current-lang');
        if (currentDisplay) {
            const langNames = {
                'en': 'English',
                'zh-CN': '简体中文',
                'zh-TW': '繁體中文'
            };
            currentDisplay.textContent = langNames[this.currentLang];
        }
    }

    onLanguageChange(callback) {
        this.listeners.push(callback);
    }

    notifyListeners() {
        this.listeners.forEach(cb => cb(this.currentLang));
    }
}

// Export
export { translations, I18n };
